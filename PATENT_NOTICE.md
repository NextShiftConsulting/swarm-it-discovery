# Patent Notice

This software implements technology that is the subject of US Patent Application
No. 19/575,615, "Methods and Systems for Structured Compatibility Certification
and Execution Control in One or More External Systems" (patent pending).

## Patented Technology

The following inventions are covered by the patent application:

### 1. RSCT (Representation-Space Compatibility Theory)
A novel methodology for certifying AI/LLM execution quality through representation-solver compatibility analysis.

**Key innovations:**
- Compatibility scoring between semantic representations and solver capabilities
- Quality certification methodology for AI outputs
- Predictive quality metrics based on representation-solver fit

### 2. κ-Gate Enforcement System
A quality gate system for AI/LLM governance using hierarchical kappa metrics.

**Key innovations:**
- Minimum quality thresholds (κ-gate) for AI execution approval
- Multi-level quality gates for different execution contexts
- Automatic execution blocking based on compatibility scores

### 3. Multi-Agent Swarm Certification
Consensus-based certification protocol for agent-to-agent communications.

**Key innovations:**
- Distributed certification across agent swarms
- Byzantine-fault-tolerant consensus scoring
- Agent reputation and trust propagation

### 4. Multimodal Kappa Decomposition
Hierarchical compatibility scoring for multimodal AI systems.

**Key innovations:**
- Decomposition into κ_H (high-level), κ_L (low-level), κ_interface
- Cross-modal compatibility analysis
- Hierarchical quality gate enforcement

## License Structure

This software is licensed under **Apache License 2.0** with **commercial use restrictions** for patented technology.

```
┌─────────────────────────────────────────────┐
│ Apache License 2.0                          │
│ ├─ Copyright grant (full rights)            │
│ ├─ Patent grant (non-commercial use)        │
│ └─ Defensive termination (patent protection)│
└─────────────────────────────────────────────┘
                    +
┌─────────────────────────────────────────────┐
│ Commercial License Required                 │
│ └─ For commercial use of patented RSCT tech │
└─────────────────────────────────────────────┘
```

### What This Means

**✓ You CAN (under Apache 2.0):**
- Use for research and education
- Modify and create derivatives
- Contribute improvements
- Evaluate for commercial assessment
- Use in open source projects (non-commercial)

**⚠️ Commercial Use Requires Additional License:**
- Production deployment in commercial products
- SaaS/API offerings using RSCT
- Commercial consulting or professional services
- Selling products incorporating this technology

See [COMMERCIAL_USE.md](COMMERCIAL_USE.md) for complete details.

## Patent Grant (Apache 2.0)

Section 3 of the Apache License 2.0 grants:

> "...a perpetual, worldwide, non-exclusive, no-charge, royalty-free,
> irrevocable (except as stated in this section) patent license to make,
> have made, use, offer to sell, sell, import, and otherwise transfer
> the Work..."

**With defensive termination:**

> "If You institute patent litigation against any entity...alleging
> that the Work...constitutes direct or contributory patent infringement,
> then any patent licenses granted to You under this License for that
> Work shall terminate..."

This provides strong protection against patent trolls while allowing legitimate use.

## Commercial Patent License

For commercial use of the patent-pending RSCT technology, a separate commercial license may be required. This includes:

- **Extended patent grant** for production use
- **Indemnification** options
- **Support and maintenance**
- **Upgrade rights** to future versions

Contact: [legal@nextshiftconsulting.com](mailto:legal@nextshiftconsulting.com)

## Contributions

By contributing to this repository, you agree that:

1. Your contributions are licensed under Apache License 2.0
2. You grant Next Shift Consulting LLC the right to use your contributions in commercial licenses
3. You have the right to make such contributions (no conflicting obligations)
4. You have not knowingly included any patented technology without disclosure

See [CONTRIBUTING.md](CONTRIBUTING.md) for the complete Contributor License Agreement (CLA).

## Prior Art and Citations

If you use this technology in academic or commercial contexts, please cite:

```bibtex
@software{nextshift2026rsct,
  author = {Next Shift Consulting LLC},
  title = {RSCT: Representation-Space Compatibility Theory for AI Governance},
  year = {2026},
  note = {US Patent Application No. 19/575,615, patent pending},
  url = {https://github.com/NextShiftConsulting/swarm-it-api}
}
```

## Patent Status

- **Inventor**: Rudolph A Martin
- **Assignee**: Next Shift Consulting LLC
- **Application Number**: US 19/575,615
- **Status**: Patent Pending

## Licensing Inquiries

For questions about:
- **Commercial licensing**: legal@nextshiftconsulting.com
- **Patent licensing**: legal@nextshiftconsulting.com
- **Academic use**: research@nextshiftconsulting.com
- **General information**: info@nextshiftconsulting.com

## Legal Notices

This patent notice is provided for informational purposes and does not constitute legal advice.

**No implied licenses**: No license is granted except as expressly stated in the LICENSE file or in a separate written agreement with Next Shift Consulting LLC.

**Export control**: This software may be subject to U.S. export control regulations. Users are responsible for compliance with applicable export laws.

---

© 2026 Next Shift Consulting LLC. All rights reserved.

"RSCT", "Representation-Space Compatibility Theory", "Swarm-It", and related marks are trademarks or registered trademarks of Next Shift Consulting LLC.
